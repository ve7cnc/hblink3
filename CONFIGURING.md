...
| `RSSI_TRAILER` | `True` appends the Homebrew BER/RSSI bytes to every outgoing DMRD — a 55-byte body with the HMAC computed over all 55 — so a peer can see the source repeater's RSSI. **Non-standard: set it only for a peer that expects it** (e.g. cc2obp, which passes the RSSI on to a c-Bridge). Inbound frames are unaffected. Default `False`. |
