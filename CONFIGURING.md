...
| `RSSI_TRAILER` | `True` appends the Homebrew BER/RSSI bytes to every outgoing DMRD — a 55-byte body with the HMAC computed over all 55 — so a peer can see the source repeater's RSSI. It also accepts that 55-byte form inbound and reports an RSSI carried on a call's terminator (cc2obp with `rssi_trailer` relays the c-Bridge's end-of-call RSSI this way). **Non-standard: set it only for a peer that expects it** (e.g. cc2obp). Standard 53-byte frames are still accepted. Default `False`. |
